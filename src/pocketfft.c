...
/*
 * floating point version is added by LTE Inc. and does not tested well.
...
struct cfft_plan_i_f;
typedef struct cfft_plan_i_f * cfft_plan_f;
struct rfft_plan_i_f;
typedef struct rfft_plan_i_f * rfft_plan_f;
...
static void my_sincosm1pi_f (float a, float *restrict res)
  {
  float s = a * a;
  /* Approximate cos(pi*x)-1 for x in [-0.25,0.25] */
...
NOINLINE static void calc_first_octant_f(size_t den, float * restrict res)
  {
  size_t n = (den+4)>>3;
  if (n==0) return;
  res[0]=1.; res[1]=0.;
  ...
  size_t l1=(size_t)sqrt(n);
  for (size_t i=1; i<l1; ++i)
    my_sincosm1pi_f((2.*i)/den,&res[2*i]);
  size_t start=l1;
  while(start<n)
    {
    float cs[2];
    my_sincosm1pi_f((2.*start)/den,cs);
    res[2*start] = cs[0]+1.;
    res[2*start+1] = cs[1];
    size_t end = l1;
    if (start+end>n) end = n-start;
...
NOINLINE static void calc_first_quadrant_f(size_t n, float * restrict res)
  {
  float * restrict p = res+n;
  calc_first_octant_f(n<<1, p);
  size_t ndone=(n+2)>>2;
  size_t i=0, idx1=0, idx2=2*ndone-2;
  for (; i+1<ndone; i+=2, idx1+=2, idx2-=2)
    {
...
NOINLINE static void calc_first_half_f(size_t n, float * restrict res)
  {
  int ndone=(n+1)>>1;
  float * p = res+n-1;
  calc_first_octant_f(n<<2, p);
  int i4=0, in=n, i=0;
  for (; i4<=in-i4; ++i, i4+=4) // octant 0
    {
    res[2*i] = p[2*i4]; res[2*i+1] = p[2*i4+1];
...
NOINLINE static void fill_first_quadrant_f(size_t n, float * restrict res)
  {
...
NOINLINE static void fill_first_half_f(size_t n, float * restrict res)
  {
  size_t half = n>>1;
  if ((n&3)==0)
    for (size_t i=0; i<half; i+=2)
...
NOINLINE static void fill_second_half_f(size_t n, float * restrict res)
  {
  if ((n&1)==0)
    for (size_t i=0; i<n; ++i)
      res[i+n] = -res[i];
...
NOINLINE static void sincos_2pibyn_half_f(size_t n, float * restrict res)
  {
  if ((n&3)==0)
    {
    calc_first_octant_f(n, res);
    fill_first_quadrant_f(n, res);
    fill_first_half_f(n, res);
    }
  else if ((n&1)==0)
    {
    calc_first_quadrant_f(n, res);
    fill_first_half_f(n, res);
    }
  else
    calc_first_half_f(n, res);
  }
...
NOINLINE static void sincos_2pibyn_f(size_t n, float * restrict res)
  {
  sincos_2pibyn_half_f(n, res);
  fill_second_half_f(n, res);
  }
...
NOINLINE static float cost_guess_f (size_t n)
  {
  ...
  size_t limit=(size_t)sqrtf(n+0.01);
  for (size_t x=3; x<=limit; x+=2)
  while ((tmp=(n/x))*x==n)
    {
    result+= (x<=5) ? x : lfp*x; // penalize larger prime factors
...
typedef struct cfftp_fctdata_f
  {
  size_t fct;
  cmplx_f *tw, *tws;
  } cfftp_fctdata_f;
...
typedef struct cfftp_plan_i_f
  {
  size_t length, nfct;
  cmplx_f *mem;
  cfftp_fctdata_f fct[NFCT];
  } cfftp_plan_i_f;
typedef struct cfftp_plan_i_f * cfftp_plan_f;
...
NOINLINE static void pass2b_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa)
  {
  const size_t cdim=2;
...
NOINLINE static void pass2f_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa)
  {
  const size_t cdim=2;
...
#define PARTSTEP3a_f(u1,u2,twr,twi) \
        { \
        cmplx_f ca,cb; \
        ca.r=t0.r+twr*t1.r; \
        ca.i=t0.i+twr*t1.i; \
...
#define PARTSTEP3b_f(u1,u2,twr,twi) \
        { \
        cmplx_f ca,cb,da,db; \
        ca.r=t0.r+twr*t1.r; \
        ca.i=t0.i+twr*t1.i; \
...
NOINLINE static void pass3b_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa)
  {
  const size_t cdim=3;
...
#define PARTSTEP3f_f(u1,u2,twr,twi) \
        { \
        cmplx_f ca,cb,da,db; \
        ca.r=t0.r+twr*t1.r; \
        ca.i=t0.i+twr*t1.i; \
...
NOINLINE static void pass3f_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa)
  {
  const size_t cdim=3;
  const float tw1r=-0.5, tw1i= -0.86602540378443864676;
...
NOINLINE static void pass4b_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa)
  {
  const size_t cdim=4;
  ...
  if (ido==1)
    ...
    for (size_t k=0; k<l1; ++k)
      ...
      for (size_t i=1; i<ido; ++i)
        {
        cmplx_f c2, c3, c4, t1, t2, t3, t4;
        cmplx_f cc0=CC(i,0,k), cc1=CC(i,1,k),cc2=CC(i,2,k),cc3=CC(i,3,k);
        PMC(t2,t1,cc0,cc2)
        PMC(t3,t4,cc1,cc3)
        ROT90(t4)
        cmplx_f wa0=WA(0,i), wa1=WA(1,i),wa2=WA(2,i);
        PMC(CH(i,k,0),c3,t2,t3)
        PMC(c2,c4,t1,t4)
        A_EQ_B_MUL_C (CH(i,k,1),wa0,c2)
        A_EQ_B_MUL_C (CH(i,k,2),wa1,c3)
...
NOINLINE static void pass4f_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa)
  {
  const size_t cdim=4;
...
#define PREP5_f(idx) \
        cmplx_f t0 = CC(idx,0,k), t1, t2, t3, t4; \
        PMC (t1,t4,CC(idx,1,k),CC(idx,4,k)) \
        PMC (t2,t3,CC(idx,2,k),CC(idx,3,k)) \
        CH(idx,k,0).r=t0.r+t1.r+t2.r; \
        CH(idx,k,0).i=t0.i+t1.i+t2.i;

#define PARTSTEP5a_f(u1,u2,twar,twbr,twai,twbi) \
        { \
        cmplx_f ca,cb; \
        ca.r=t0.r+twar*t1.r+twbr*t2.r; \
        ca.i=t0.i+twar*t1.i+twbr*t2.i; \
...
#define PARTSTEP5b_f(u1,u2,twar,twbr,twai,twbi) \
        { \
        cmplx_f ca,cb,da,db; \
        ca.r=t0.r+twar*t1.r+twbr*t2.r; \
        ca.i=t0.i+twar*t1.i+twbr*t2.i; \
...
NOINLINE static void pass5b_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa)
  {
  const size_t cdim=5;
  ...
  if (ido==1)
    for (size_t k=0; k<l1; ++k)
      {
      PREP5_f(0)
      PARTSTEP5a_f(1,4,tw1r,tw2r,+tw1i,+tw2i)
      PARTSTEP5a_f(2,3,tw2r,tw1r,+tw2i,-tw1i)
      }
  else
    for (size_t k=0; k<l1; ++k)
      {
      ...
      for (size_t i=1; i<ido; ++i)
        {
        PREP5_f(i)
        PARTSTEP5b_f(1,4,tw1r,tw2r,+tw1i,+tw2i)
        PARTSTEP5b_f(2,3,tw2r,tw1r,+tw2i,-tw1i)
        }
      }
  }
...
#define PARTSTEP5f_f(u1,u2,twar,twbr,twai,twbi) \
        { \
        cmplx_f ca,cb,da,db; \
        ca.r=t0.r+twar*t1.r+twbr*t2.r; \
        ca.i=t0.i+twar*t1.i+twbr*t2.i; \
...
NOINLINE static void pass5f_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa)
  {
  const size_t cdim=5;
  const float tw1r= 0.3090169943749474241,
  ...
  if (ido==1)
    ...
    for (size_t k=0; k<l1; ++k)
      ...
      for (size_t i=1; i<ido; ++i)
        {
        PREP5_f(i)
        PARTSTEP5f_f(1,4,tw1r,tw2r,+tw1i,+tw2i)
        PARTSTEP5f_f(2,3,tw2r,tw1r,+tw2i,-tw1i)
        }
      }
  }
...
#define PREP7_f(idx) \
        cmplx_f t1 = CC(idx,0,k), t2, t3, t4, t5, t6, t7; \
        PMC (t2,t7,CC(idx,1,k),CC(idx,6,k)) \
        PMC (t3,t6,CC(idx,2,k),CC(idx,5,k)) \
        PMC (t4,t5,CC(idx,3,k),CC(idx,4,k)) \
        CH(idx,k,0).r=t1.r+t2.r+t3.r+t4.r; \
...
#define PARTSTEP7a0_f(u1,u2,x1,x2,x3,y1,y2,y3,out1,out2) \
        { \
        cmplx_f ca,cb; \
        ca.r=t1.r+x1*t2.r+x2*t3.r+x3*t4.r; \
        ca.i=t1.i+x1*t2.i+x2*t3.i+x3*t4.i; \
...
#define PARTSTEP7a_f(u1,u2,x1,x2,x3,y1,y2,y3) \
        PARTSTEP7a0_f(u1,u2,x1,x2,x3,y1,y2,y3,CH(0,k,u1),CH(0,k,u2))
#define PARTSTEP7_f(u1,u2,x1,x2,x3,y1,y2,y3) \
        { \
        cmplx_f da,db; \
        PARTSTEP7a0_f(u1,u2,x1,x2,x3,y1,y2,y3,da,db) \
        MULPMSIGNC (CH(i,k,u1),WA(u1-1,i),da) \
        MULPMSIGNC (CH(i,k,u2),WA(u2-1,i),db) \
        }

NOINLINE static void pass7_f(size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa, const int sign)
  {
  const size_t cdim=7;
  ...
  if (ido==1)
    for (size_t k=0; k<l1; ++k)
      {
      PREP7_f(0)
      PARTSTEP7a_f(1,6,tw1r,tw2r,tw3r,+tw1i,+tw2i,+tw3i)
      PARTSTEP7a_f(2,5,tw2r,tw3r,tw1r,+tw2i,-tw3i,-tw1i)
      PARTSTEP7a_f(3,4,tw3r,tw1r,tw2r,+tw3i,-tw1i,+tw2i)
      }
  else
    for (size_t k=0; k<l1; ++k)
      {
      ...
      for (size_t i=1; i<ido; ++i)
        {
        PREP7_f(i)
        PARTSTEP7_f(1,6,tw1r,tw2r,tw3r,+tw1i,+tw2i,+tw3i)
        PARTSTEP7_f(2,5,tw2r,tw3r,tw1r,+tw2i,-tw3i,-tw1i)
        PARTSTEP7_f(3,4,tw3r,tw1r,tw2r,+tw3i,-tw1i,+tw2i)
        }
      }
  }
...
#define PREP11_f(idx) \
        cmplx_f t1 = CC(idx,0,k), t2, t3, t4, t5, t6, t7, t8, t9, t10, t11; \
        PMC (t2,t11,CC(idx,1,k),CC(idx,10,k)) \
        PMC (t3,t10,CC(idx,2,k),CC(idx, 9,k)) \
        PMC (t4,t9 ,CC(idx,3,k),CC(idx, 8,k)) \
        PMC (t5,t8 ,CC(idx,4,k),CC(idx, 7,k)) \
...
#define PARTSTEP11a0_f(u1,u2,x1,x2,x3,x4,x5,y1,y2,y3,y4,y5,out1,out2) \
        { \
        cmplx_f ca,cb; \
        ca.r=t1.r+x1*t2.r+x2*t3.r+x3*t4.r+x4*t5.r+x5*t6.r; \
        ca.i=t1.i+x1*t2.i+x2*t3.i+x3*t4.i+x4*t5.i+x5*t6.i; \
...
#define PARTSTEP11a_f(u1,u2,x1,x2,x3,x4,x5,y1,y2,y3,y4,y5) \
        PARTSTEP11a0_f(u1,u2,x1,x2,x3,x4,x5,y1,y2,y3,y4,y5,CH(0,k,u1),CH(0,k,u2))
#define PARTSTEP11_f(u1,u2,x1,x2,x3,x4,x5,y1,y2,y3,y4,y5) \
        { \
        cmplx_f da,db; \
        PARTSTEP11a0_f(u1,u2,x1,x2,x3,x4,x5,y1,y2,y3,y4,y5,da,db) \
        MULPMSIGNC (CH(i,k,u1),WA(u1-1,i),da) \
        MULPMSIGNC (CH(i,k,u2),WA(u2-1,i),db) \
        }

NOINLINE static void pass11_f (size_t ido, size_t l1, const cmplx_f * restrict cc,
  cmplx_f * restrict ch, const cmplx_f * restrict wa, const int sign)
  {
  const size_t cdim=11;
  ...
  if (ido==1)
    for (size_t k=0; k<l1; ++k)
      {
      PREP11_f(0)
      PARTSTEP11a_f(1,10,tw1r,tw2r,tw3r,tw4r,tw5r,+tw1i,+tw2i,+tw3i,+tw4i,+tw5i)
      PARTSTEP11a_f(2, 9,tw2r,tw4r,tw5r,tw3r,tw1r,+tw2i,+tw4i,-tw5i,-tw3i,-tw1i)
      PARTSTEP11a_f(3, 8,tw3r,tw5r,tw2r,tw1r,tw4r,+tw3i,-tw5i,-tw2i,+tw1i,+tw4i)
      PARTSTEP11a_f(4, 7,tw4r,tw3r,tw1r,tw5r,tw2r,+tw4i,-tw3i,+tw1i,+tw5i,-tw2i)
      PARTSTEP11a_f(5, 6,tw5r,tw1r,tw4r,tw2r,tw3r,+tw5i,-tw1i,+tw4i,-tw2i,+tw3i)
      }
  else
    for (size_t k=0; k<l1; ++k)
      {
      ...
      for (size_t i=1; i<ido; ++i)
        {
        PREP11_f(i)
        PARTSTEP11_f(1,10,tw1r,tw2r,tw3r,tw4r,tw5r,+tw1i,+tw2i,+tw3i,+tw4i,+tw5i)
        PARTSTEP11_f(2, 9,tw2r,tw4r,tw5r,tw3r,tw1r,+tw2i,+tw4i,-tw5i,-tw3i,-tw1i)
        PARTSTEP11_f(3, 8,tw3r,tw5r,tw2r,tw1r,tw4r,+tw3i,-tw5i,-tw2i,+tw1i,+tw4i)
        PARTSTEP11_f(4, 7,tw4r,tw3r,tw1r,tw5r,tw2r,+tw4i,-tw3i,+tw1i,+tw5i,-tw2i)
        PARTSTEP11_f(5, 6,tw5r,tw1r,tw4r,tw2r,tw3r,+tw5i,-tw1i,+tw4i,-tw2i,+tw3i)
        }
      }
  }
...
NOINLINE static int passg_f (size_t ido, size_t ip, size_t l1,
  cmplx_f * restrict cc, cmplx_f * restrict ch, const cmplx_f * restrict wa,
  const cmplx_f * restrict csarr, const int sign)
  {
  const size_t cdim=ip;
  size_t ipph = (ip+1)/2;
  size_t idl1 = ido*l1;

  cmplx_f * restrict wal=RALLOC(cmplx_f,ip);
  if (!wal) return -1;
  wal[0]=(cmplx_f){1.,0.};
  for (size_t i=1; i<ip; ++i)
    wal[i]=(cmplx_f){csarr[i].r,sign*csarr[i].i};
  ...
  for (size_t l=1, lc=ip-1; l<ipph; ++l, --lc)
    ...
    for (; j<ipph-1; j+=2, jc-=2)
      {
      iwal+=l; if (iwal>ip) iwal-=ip;
      cmplx_f xwal=wal[iwal];
      iwal+=l; if (iwal>ip) iwal-=ip;
      cmplx_f xwal2=wal[iwal];
      for (size_t ik=0; ik<idl1; ++ik)
        {
        CX2(ik,l).r += CH2(ik,j).r*xwal.r+CH2(ik,j+1).r*xwal2.r;
        CX2(ik,l).i += CH2(ik,j).i*xwal.r+CH2(ik,j+1).i*xwal2.r;
  ...
  if (ido==1)
    for (size_t j=1, jc=ip-1; j<ipph; ++j, --jc)
      for (size_t ik=0; ik<idl1; ++ik)
        {
        cmplx_f t1=CX2(ik,j), t2=CX2(ik,jc);
        PMC(CX2(ik,j),CX2(ik,jc),t1,t2)
        }
  else
    {
    for (size_t j=1, jc=ip-1; j<ipph; ++j,--jc)
      for (size_t k=0; k<l1; ++k)
        {
        cmplx_f t1=CX(0,k,j), t2=CX(0,k,jc);
        PMC(CX(0,k,j),CX(0,k,jc),t1,t2)
        for (size_t i=1; i<ido; ++i)
          {
          cmplx_f x1, x2;
...
NOINLINE WARN_UNUSED_RESULT static int pass_all_f(cfftp_plan_f plan, cmplx_f c[], float fct,
  const int sign)
  {
  if (plan->length==1) return 0;
  size_t len=plan->length;
  size_t l1=1, nf=plan->nfct;
  cmplx_f *ch = RALLOC(cmplx_f, len);
  if (!ch) return -1;
  cmplx_f *p1=c, *p2=ch;
  ...
  for(size_t k1=0; k1<nf; k1++)
    ...
    size_t ido = len/l2;
    if     (ip==4)
      sign>0 ? pass4b_f (ido, l1, p1, p2, plan->fct[k1].tw)
             : pass4f_f (ido, l1, p1, p2, plan->fct[k1].tw);
    else if(ip==2)
      sign>0 ? pass2b_f (ido, l1, p1, p2, plan->fct[k1].tw)
             : pass2f_f (ido, l1, p1, p2, plan->fct[k1].tw);
    else if(ip==3)
      sign>0 ? pass3b_f (ido, l1, p1, p2, plan->fct[k1].tw)
             : pass3f_f (ido, l1, p1, p2, plan->fct[k1].tw);
    else if(ip==5)
      sign>0 ? pass5b_f (ido, l1, p1, p2, plan->fct[k1].tw)
             : pass5f_f (ido, l1, p1, p2, plan->fct[k1].tw);
    else if(ip==7)  pass7_f (ido, l1, p1, p2, plan->fct[k1].tw, sign);
    else if(ip==11) pass11_f(ido, l1, p1, p2, plan->fct[k1].tw, sign);
    else
      {
      if (passg_f(ido, ip, l1, p1, p2, plan->fct[k1].tw, plan->fct[k1].tws, sign))
        { DEALLOC(ch); return -1; }
      SWAP(p1,p2,cmplx_f *);
      }
    SWAP(p1,p2,cmplx_f *);
  ...
  if (p1!=c)
    ...
    if (fct!=1.)
      ...
      memcpy (c,p1,len*sizeof(cmplx_f));
    }
  else
    if (fct!=1.)
      for (size_t i=0; i<len; ++i)
...
NOINLINE WARN_UNUSED_RESULT
static int cfftp_forward_f(cfftp_plan_f plan, float c[], float fct)
  { return pass_all_f(plan,(cmplx_f *)c, fct, -1); }
...
NOINLINE WARN_UNUSED_RESULT
static int cfftp_backward_f(cfftp_plan_f plan, float c[], float fct)
  { return pass_all_f(plan,(cmplx_f *)c, fct, 1); }
...
NOINLINE WARN_UNUSED_RESULT
static int cfftp_factorize_f (cfftp_plan_f plan)
  {
  size_t length=plan->length;
  size_t nfct=0;
  while ((length%4)==0)
  ...
  size_t maxl=(size_t)(sqrtf((float)length))+1;
  for (size_t divisor=3; (length>1)&&(divisor<maxl); divisor+=2)
    if ((length%divisor)==0)
      {
      while ((length%divisor)==0)
      ...
      maxl=(size_t)(sqrtf((float)length))+1;
      }
  if (length>1) plan->fct[nfct++].fct=length;
  plan->nfct=nfct;
  return 0;
...
NOINLINE static size_t cfftp_twsize_f (cfftp_plan_f plan)
  {
  size_t twsize=0, l1=1;
  for (size_t k=0; k<plan->nfct; ++k)
    {
...
NOINLINE WARN_UNUSED_RESULT static int cfftp_comp_twiddle_f (cfftp_plan_f plan)
  {
  size_t length=plan->length;
  float *twid = RALLOC(float, 2*length);
  if (!twid) return -1;
  sincos_2pibyn_f(length, twid);
  size_t l1=1;
  size_t memofs=0;
  for (size_t k=0; k<plan->nfct; ++k)
    {
...
static cfftp_plan_f make_cfftp_plan_f (size_t length)
  {
  if (length==0) return NULL;
  cfftp_plan_f plan = RALLOC(cfftp_plan_i_f,1);
  if (!plan) return NULL;
  plan->length=length;
  plan->nfct=0;
  for (size_t i=0; i<NFCT; ++i)
    plan->fct[i]=(cfftp_fctdata_f){0,0,0};
  plan->mem=0;
  if (length==1) return plan;
  if (cfftp_factorize_f(plan)!=0) { DEALLOC(plan); return NULL; }
  size_t tws=cfftp_twsize_f(plan);
  plan->mem=RALLOC(cmplx_f,tws);
  if (!plan->mem) { DEALLOC(plan); return NULL; }
  if (cfftp_comp_twiddle_f(plan)!=0)
    { DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  return plan;
  }
...
static void destroy_cfftp_plan_f (cfftp_plan_f plan)
  {
  DEALLOC(plan->mem);
  DEALLOC(plan);
  }
...
typedef struct rfftp_fctdata_f
  {
  size_t fct;
  float *tw, *tws;
  } rfftp_fctdata_f;
...
typedef struct rfftp_plan_i_f
  {
  size_t length, nfct;
  float *mem;
  rfftp_fctdata_f fct[NFCT];
  } rfftp_plan_i_f;
typedef struct rfftp_plan_i_f * rfftp_plan_f;
...
NOINLINE static void radf2_f (size_t ido, size_t l1, const float * restrict cc,
...
NOINLINE static void radf3_f(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=3;
...
NOINLINE static void radf4_f(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=4;
  static const float hsqt2=0.70710678118654752440f;
...
NOINLINE static void radf5_f(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=5;
...
NOINLINE static void radfg_f(size_t ido, size_t ip, size_t l1,
...
static void copy_and_norm_f(float *c, float *p1, size_t n, double fct)
  {
  if (p1!=c)
    {
    if (fct!=1.f)
...
WARN_UNUSED_RESULT
static int rfftp_forward_f(rfftp_plan_f plan, float c[], float fct)
  {
  if (plan->length==1) return 0;
  size_t n=plan->length;
  size_t l1=n, nf=plan->nfct;
  ...
  for(size_t k1=0; k1<nf;++k1)
    ...
    l1 /= ip;
    if(ip==4)
      radf4_f(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==2)
      radf2_f(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==3)
      radf3_f(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==5)
      radf5_f(ido, l1, p1, p2, plan->fct[k].tw);
    else
      {
      radfg_f(ido, ip, l1, p1, p2, plan->fct[k].tw, plan->fct[k].tws);
      SWAP (p1,p2,float *);
      }
    SWAP (p1,p2,float *);
    }
  copy_and_norm_f(c,p1,n,fct);
  DEALLOC(ch);
  return 0;
  }
...
WARN_UNUSED_RESULT
static int rfftp_factorize_f (rfftp_plan_f plan)
  {
  size_t length=plan->length;
  size_t nfct=0;
  while ((length%4)==0)
...
WARN_UNUSED_RESULT NOINLINE static int rfftp_comp_twiddle_f (rfftp_plan_f plan)
  {
  size_t length=plan->length;
  float *twid = RALLOC(float, 2*length);
  if (!twid) return -1;
  sincos_2pibyn_half_f(length, twid);
  size_t l1=1;
  float *ptr=plan->mem;
  for (size_t k=0; k<plan->nfct; ++k)
    {
...
NOINLINE static rfftp_plan_f make_rfftp_plan_f (size_t length)
  {
  if (length==0) return NULL;
  rfftp_plan_f plan = RALLOC(rfftp_plan_i_f,1);
  if (!plan) return NULL;
  plan->length=length;
  plan->nfct=0;
  plan->mem=NULL;
  for (size_t i=0; i<NFCT; ++i)
    plan->fct[i]=(rfftp_fctdata_f){0,0,0};
  if (length==1) return plan;
  if (rfftp_factorize_f(plan)!=0) { DEALLOC(plan); return NULL; }
  size_t tws=rfftp_twsize_f(plan);
  plan->mem=RALLOC(float,tws);
  if (!plan->mem) { DEALLOC(plan); return NULL; }
  if (rfftp_comp_twiddle_f(plan)!=0)
    { DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  return plan;
  }
...
NOINLINE static void destroy_rfftp_plan_f (rfftp_plan_f plan)
  {
  DEALLOC(plan->mem);
  DEALLOC(plan);
  }
...
typedef struct fftblue_plan_i_f
  {
  size_t n, n2;
  cfftp_plan_f plan;
  float *mem;
...
typedef struct fftblue_plan_i_f * fftblue_plan_f;
...
NOINLINE static fftblue_plan_f make_fftblue_plan_f (size_t length)
  {
  fftblue_plan_f plan = RALLOC(fftblue_plan_i_f,1);
  if (!plan) return NULL;
  plan->n = length;
  plan->n2 = good_size(plan->n*2-1);
...
/* initialize b_k */
  ...
  for (size_t m=2*plan->n;m<=(2*plan->n2-2*plan->n+1);++m)
    plan->bkf[m]=0.;
  plan->plan=make_cfftp_plan_f(plan->n2);
  if (!plan->plan)
    { DEALLOC(tmp); DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  if (cfftp_forward_f(plan->plan,plan->bkf,1.)!=0)
    { DEALLOC(tmp); DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  DEALLOC(tmp);
...
NOINLINE static void destroy_fftblue_plan_f (fftblue_plan_f plan)
  {
  DEALLOC(plan->mem);
  destroy_cfftp_plan_f(plan->plan);
  DEALLOC(plan);
  }
...
NOINLINE WARN_UNUSED_RESULT
static int fftblue_fft_f(fftblue_plan_f plan, float c[], int isign, float fct)
  {
  size_t n=plan->n;
  size_t n2=plan->n2;
  float *bk  = plan->bk;
...
/* initialize a_k and FFT it */
  ...
  if (cfftp_forward_f (plan->plan,akf,fct)!=0)
    { DEALLOC(akf); return -1; }
...
/* inverse FFT */
  if (cfftp_backward_f (plan->plan,akf,1.)!=0)
    { DEALLOC(akf); return -1; }
...
WARN_UNUSED_RESULT
static int cfftblue_backward_f(fftblue_plan_f plan, float c[], float fct)
  { return fftblue_fft_f(plan,c,1,fct); }
...
WARN_UNUSED_RESULT
static int cfftblue_forward_f(fftblue_plan_f plan, float c[], float fct)
  { return fftblue_fft_f(plan,c,-1,fct); }
...
WARN_UNUSED_RESULT
static int rfftblue_backward_f(fftblue_plan_f plan, float c[], float fct)
  {
  size_t n=plan->n;
  ...
  if (fftblue_fft_f(plan,tmp,1,fct)!=0)
    { DEALLOC(tmp); return -1; }
  for (size_t m=0; m<n; ++m)
    c[m] = tmp[2*m];
  DEALLOC(tmp);
...
WARN_UNUSED_RESULT
static int rfftblue_forward_f(fftblue_plan_f plan, float c[], float fct)
  {
  size_t n=plan->n;
  float *tmp = RALLOC(float,2*n);
  if (!tmp) return -1;
...
typedef struct cfft_plan_i_f
  {
  cfftp_plan_f packplan;
  fftblue_plan_f blueplan;
  } cfft_plan_i_f;
...
static cfft_plan_f make_cfft_plan_f (size_t length)
  {
  if (length==0) return NULL;
  cfft_plan_f plan = RALLOC(cfft_plan_i_f,1);
  if (!plan) return NULL;
  plan->blueplan=0;
  plan->packplan=0;
  if ((length<50) || (largest_prime_factor(length)<=sqrt(length)))
    {
    plan->packplan=make_cfftp_plan_f(length);
    if (!plan->packplan) { DEALLOC(plan); return NULL; }
    return plan;
    }
  ...
  if (comp2<comp1) // use Bluestein
    {
    plan->blueplan=make_fftblue_plan_f(length);
    if (!plan->blueplan) { DEALLOC(plan); return NULL; }
    }
  else
    {
...
WARN_UNUSED_RESULT static int cfft_backward_f(cfft_plan_f plan, float c[], float fct)
  {
  if (plan->packplan)
    return cfftp_backward_f(plan->packplan,c,fct);
  // if (plan->blueplan)
  return cfftblue_backward_f(plan->blueplan,c,fct);
  }
...
WARN_UNUSED_RESULT static int cfft_forward_f(cfft_plan_f plan, float c[], float fct)
  {
  if (plan->packplan)
    return cfftp_forward_f(plan->packplan,c,fct);
  // if (plan->blueplan)
  return cfftblue_forward_f(plan->blueplan,c,fct);
  }
...
typedef struct rfft_plan_i_f
  {
  rfftp_plan_f packplan;
  fftblue_plan_f blueplan;
  } rfft_plan_i_f;
...
static rfft_plan_f make_rfft_plan_f (size_t length)
  {
  if (length==0) return NULL;
  rfft_plan_f plan = RALLOC(rfft_plan_i_f,1);
  if (!plan) return NULL;
  plan->blueplan=0;
  plan->packplan=0;
  if ((length<50) || (largest_prime_factor(length)<=sqrt(length)))
    {
    plan->packplan=make_rfftp_plan_f(length);
    if (!plan->packplan) { DEALLOC(plan); return NULL; }
    return plan;
    }
  double comp1 = 0.5*cost_guess(length);
...
static void destroy_rfft_plan_f (rfft_plan_f plan)
  {
  if (plan->blueplan)
    destroy_fftblue_plan_f(plan->blueplan);
  if (plan->packplan)
    destroy_rfftp_plan_f(plan->packplan);
  DEALLOC(plan);
  }
...
WARN_UNUSED_RESULT static int rfft_forward_f(rfft_plan_f plan, float c[], float fct)
  {
  if (plan->packplan)
    return rfftp_forward_f(plan->packplan,c,fct);
  else // if (plan->blueplan)
    return rfftblue_forward_f(plan->blueplan,c,fct);
  }
